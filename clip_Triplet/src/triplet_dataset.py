...
class TripletDataset(Dataset):
    def __init__(self, root_dir, transform=None, allowed_classes=None):
        ...
        if allowed_classes is not None:
            self.class_to_images = {cls: imgs for cls, imgs in self.class_to_images.items() if cls in allowed_classes}

        self.valid_classes = [cls for cls, imgs in self.class_to_images.items() if len(imgs) >= 2]
        if not self.valid_classes:
            raise ValueError("No class with at least 2 images found.")
